/**
 * session.js — Cookie & localStorage session persistence for Kronos Workforce Central / UKG Pro
...
async function loadSession(page) {
  ...
  if (session.localStorage && Object.keys(session.localStorage).length > 0) {
    ...
    const url = new URL(session.url || process.env.KRONOS_UKG_URL || 'https://localhost');
