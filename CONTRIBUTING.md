# Contributing to Kronos Workforce Central / UKG Pro Browser Automation
...
- **Broken selectors**: Kronos Workforce Central / UKG Pro updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Kronos Workforce Central / UKG Pro instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.KRONOS_UKG_URL}/path/to/section`);
...
When you find working selectors for Kronos Workforce Central / UKG Pro, document them:

```javascript
// Kronos Workforce Central / UKG Pro selectors (verified YYYY-MM-DD)
...
Kronos Workforce Central / UKG Pro updates its UI periodically. When selectors break:
