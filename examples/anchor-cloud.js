/**
 * anchor-cloud.js — AnchorBrowser cloud example for Kronos Workforce Central / UKG Pro
 *
...
const { login_kronos, approve_timesheets } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Kronos Workforce Central / UKG Pro automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_kronos(page, {});
      log('Action 1 result:', data1);

      const data2 = await approve_timesheets(page, {});
